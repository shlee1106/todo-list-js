...
function writeTodo(){  //ul클래스 js-todo-list안에 li목록 input,span,button 기능 넣기 및 사용자 입력 값 화면 출력
    let todoHTML = ''
...
function addTodo() { //사용자가 input에 입력한 값들 배열안의 객체에 저장 기능
    ...
    writeTodo() //사용자 입력값 화면 출력 및 li목록안에 다양한 버튼기능 함수호출
...
function reSet(){ //배열내의 객체 초기화
    todos = [] //reSet버튼을 누를 시 배열안의 객체 초기화
    localStorage.removeItem('todos') //todos 기록 삭제
    writeTodo() //전체 삭제된 화면 출력
}
